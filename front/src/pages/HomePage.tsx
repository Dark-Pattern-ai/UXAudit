    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        >
          ...
          <p className="text-gray-600 font-medium mb-1">금융 앱 스크린샷을 드래그하거나 클릭해 업로드</p>
          <p className="text-xs text-gray-400 mb-4">PNG · JPG · 최대 {MAX_FILES}장 · 장당 10MB 이하</p>
          <label className="inline-block px-5 py-2.5 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition-colors text-sm font-medium">
            파일 선택
      ...
      {images.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-4 overflow-x-auto pb-2">
            {images.map((img) => (
              <div key={img.id} className="relative flex-shrink-0 w-32 border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
                <button
                  onClick={() => removeImage(img.id)}
                  className="absolute top-1.5 right-1.5 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center hover:bg-red-600 transition-colors cursor-pointer z-10"
                >✕</button>
                ...
                <div className="p-2">
                  ...
                  <input
                    ...
                    placeholder="페이지명"
                    className="w-full text-xs px-2 py-1 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-700 placeholder-gray-400"
                  />
                </div>
              </div>
            ))}
            {images.length < MAX_FILES && (
              <label className="flex-shrink-0 w-32 h-[172px] border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-gray-900 truncate">{serviceName || '-'}</p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs text-gray-500 mb-1">예상 소요 시간</p>
            <p className="text-2xl font-bold text-gray-900">약 {Math.max(10, images.length * 10)}초</p>
          </div>
        </div>
      )}
