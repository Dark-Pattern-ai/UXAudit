    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <Card>
          ...
          <p className="text-4xl font-bold text-gray-900">{report.uploadedImages.length}</p>
        </Card>
      </div>
      ...
      <Card className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {report.guidelineCompliance.map((item) => {
      ...
      <div className="space-y-3">
        {report.detectedPatterns.map((pattern) => {
          const catConfig = CATEGORY_CONFIG[pattern.category as keyof typeof CATEGORY_CONFIG];
          const riskConfig = RISK_LEVEL_CONFIG[pattern.riskLevel as keyof typeof RISK_LEVEL_CONFIG];


          return (
            <div
              key={pattern.id}
              onClick={() => navigate(`/report/${report.id}/detail/${pattern.id}`)}
